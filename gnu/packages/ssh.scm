       ...
       (list
        ...
        #~(modify-phases %standard-phases
            (add-after 'unpack 'disable-documentation
              (lambda* _
                ;; There's no convenient option to turn off docs generation and
                ;; it has a lot of requirements.
                (substitute* "Makefile.in"
                  ;; Removing "doc" from both of these lists but leaving them
                  ;; otherwise the same.
                  (("^BUILD_SUBDIRS.*")
                   "BUILD_SUBDIRS = @BUILD_SUBDIRS@ src sftp")
                  (("^SUBDIRS.*")
                   "SUBDIRS = argp spki src sftp contrib misc"))))
